...
This document is for packaging, positioning, and launch.

The goal is not to sound timid. The goal is to sound bold without making claims that are easy to knock down.
...
1. It is not only a prompt pack.
2. It is not only a Zotero tutorial.
...
## Suggested GitHub repo subtitle
...
## Social post draft: Chinese

```text
我把一个“从 0 开始写综述”的工作流开源了。

不是单纯的 prompt，也不是单纯的 Zotero 教程，而是把：
主题 -> 文献检索 -> Zotero 导入 -> 证据矩阵 -> 草稿 -> Word 正式交叉引用
串成一个可以由 Codex 托管推进的流程。

重点不是让 AI 乱写，而是让 Zotero 继续做参考文献真源，让 Word 里的交叉引用最后还能正常 Refresh。

如果你写过 100+ 篇文献的综述，应该能懂这里面最痛的地方。

Repo:
https://github.com/KobeBryant0127/zotero-codex-review-workflow
```

## Show-don't-tell assets to add later

The repo will get much stronger if you add:

1. A screenshot of `codex_handoff.md`
...
1. Tighten the README hero and quick start
2. Tag a release
3. Add a screenshot or GIF
4. Post to X / LinkedIn / 小红书 / 即刻 / Linux.do / V2EX / Reddit
5. Ask a few friends to star early so the repo does not launch cold
6. Open one or two good first issues to signal momentum

## The most important narrative

Do not sell this as:

`AI writes your paper for you`

Sell this as:

`Codex manages the workflow you would otherwise have to coordinate manually, while Zotero and Word keep the references real and deliverable.`
